...
notion = Client(auth="your_notion_auth_token_here")

# Define the ID of the parent page where you want to create the new pages
parent_page_id = "your_parent_page_id_here"

# Define the path to the folder where the text files are located
folder_path = "path_to_your_text_files_here"
...
def page_exists(notion, database_id, new_title, old_title):
    filter_params = {
        "or": [
            {
                "property": "Name",
    ...
    results = notion.databases.query(database_id, filter=filter_params)
...
def generate_headline(text, max_tokens=20):
    input_text = f"Create a headline for the following text: {text}"
    ...
    while True:
        ...
        for entry in os.scandir(folder_path):
            if (entry.is_file() and
                ...
                headline_file_path = os.path.join(folder_path, f"{base_name.replace('_summary', '')}_headline.txt")

                with open(headline_file_path, "r", encoding="utf-8") as headline_file:
                    title = headline_file.read().strip()  # Always use the content of the headline.txt file as the title
                ...
                if not page_exists(notion, database_id, title, base_name) and entry.name not in processed_files:
